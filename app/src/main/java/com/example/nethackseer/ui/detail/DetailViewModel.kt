...
import com.example.nethackseer.data.local.extensions.*
...
class DetailViewModel(
    ...
    private fun loadEntity() {
        ...
        viewModelScope.launch {
            ...
            if (monster != null) {
                ...
                monster.activeAttacks.forEach {
                    flagIds.add(it.type)
                    if (it.damageType != "AD_NONE") {
                        flagIds.add(it.damageType)
                    }
                }
    ...
    private fun mapToMonsterDetails(monster: MonsterEntity, properties: List<PropertyEntity>): MonsterDetails {
        ...
        val baseConferred = MonsterData.getResolvedConferredResistances(monster)
        val specialEffects = MonsterData.getEatingSpecialEffects(monster, baseConferred)

        val isGiant = monster.isGiant
        val isMindFlayer = monster.isMindFlayer
        ...
        val conferredIntrinsics = baseConferred.map { id ->
            val isBeeOrScorpion = monster.name.contains("killer bee", ignoreCase = true) || monster.name.contains("scorpion", ignoreCase = true)
            val (num, den) = when {
        ...
        val attacksFormatted = monster.activeAttacks.map { attack ->
        ...
        val isInediate = monster.isInediate
    ...
    private fun calculateExperience(monster: MonsterEntity): Int {
        ...
        val lowAttackTypes = setOf("AT_ANY", "AT_NONE", "AT_CLAW", "AT_BITE", "AT_KICK", "AT_BUTT")
        monster.activeAttacks.forEach { attack ->
            if (!lowAttackTypes.contains(attack.type)) {
